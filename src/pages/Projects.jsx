    ...
    <div className="container-page space-y-8">
      ...
      <section className="grid gap-6 md:grid-cols-2">
        {projects.map((project) => (
          <article
            key={project.title}
            className="flex flex-col justify-between rounded-2xl bg-brand-panel p-5 shadow-sm border border-brand-border"
          >
            <div>
              <div className="h-36 rounded-xl bg-brand-panel-muted border border-brand-border flex items-center justify-center p-4">
              ...
              <div className="mt-4 flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wide text-brand-brown-light font-mono">
                  {project.type}
                </span>
              ...
              <h2 className="mt-1 text-sm font-bold text-brand-bone">{project.title}</h2>
              <p className="mt-2 text-xs text-stone-400 leading-relaxed">{project.summary}</p>
            </div>
          </article>
        ))}
      </section>
