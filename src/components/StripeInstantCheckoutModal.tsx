...
export const StripeInstantCheckoutModal: React.FC<StripeInstantCheckoutModalProps> = ({
  ...
  /**
   * Persist lead to Firestore BEFORE redirecting to Stripe.
   * Captures contact info + cart snapshot for analytics and sales follow-up
   * even if the buyer abandons the Stripe payment page.
   */
  const saveLeadToFirestore = async (orderRefId: string) => {
    if (!db) return;
    try {
      ...
      // Non-fatal — lead save failure should never block the checkout redirect
      console.warn('[Lead] Firestore lead save failed (non-fatal):', err);
    }
  };
  ...
  const handlePaySubmit = async (e: React.FormEvent) => {
    ...
    if (!buyerEmail.trim() || !buyerName.trim()) {
      setErrorMessage('Please provide your full name and work email to continue.');
      return;
    }
    if (!buyerPhone.trim()) {
      setErrorMessage('A contact phone number is required for order dispatch coordination.');
      return;
    }
    ...
    try {
      // Generate order ref locally so we can link it in Firestore before redirect
      const orderRefId = `IPG-${Date.now().toString().slice(-6)}`;

      // 1. Save lead to Firestore (analytics + follow-up safety net)
      await saveLeadToFirestore(orderRefId);

      // 2. Initiate Stripe Checkout (creates session → browser redirects to Stripe hosted page)
      ...
      if (result.success) {
        if (result.redirectUrl) {
          // Live: browser is navigating to Stripe — nothing else needed here
          return;
        } else if (result.job) {
          // Sandbox fallback: show success screen directly in modal
      ...
      setErrorMessage(err.message || 'Unable to reach payment processor. Please try again or call us directly.');
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/85 p-4 backdrop-blur-md animate-fadeIn">
      <div className="relative flex max-h-[94vh] w-full max-w-xl flex-col rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl overflow-hidden">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="rounded bg-indigo-500/20 border border-indigo-500/40 text-indigo-300 font-mono text-[10px] font-bold px-2 py-0.5 flex items-center gap-1">
                  ...
                  {isLiveStripe ? (
                    ...
                    <span className="text-[9px] text-amber-400">(SANDBOX)</span>
                  )}
                </span>
              </div>
              <p className="text-xs text-slate-400">
                Direct CNC Dispatch &bull; 256-Bit Encrypted &bull; Instant Settlement
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-auto p-6">
          {submittedJob ? (
            /* ── Success Screen (sandbox fallback only) ──────── */
            <div className="space-y-6 text-center py-6 animate-fadeIn">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-slate-300">
                  Receipt sent to <strong className="text-amber-400 font-mono">{submittedJob.buyerEmail}</strong>.
                  Reference: <strong className="font-mono text-slate-200">{submittedJob.poNumber}</strong>
                </p>
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
                <button
                  onClick={() => { onClose(); onViewShopBoard(); }}
                ...
                >
                  <span>Track on Shop Floor Whiteboard</span>
            ...
            <form onSubmit={handlePaySubmit} className="space-y-5">
              ...
              <div className="rounded-xl border border-slate-800 bg-slate-950/80 p-4 space-y-1.5">
                ...
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-400">Items ({cartItems.reduce((s, i) => s + i.quantity, 0)} units, {totalWeight.toFixed(1)} lbs):</span>
                  <span className="font-mono font-bold text-slate-200">${itemsSubtotal.toFixed(2)}</span>
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-400">Freight &amp; Handling ({shippingInfo.method}):</span>
                  <span className="font-mono text-slate-300">${shippingInfo.cost.toFixed(2)}</span>
                </div>
                {paymentType === 'card' ? (
                  ...
                  <div className="flex items-center justify-between text-xs text-emerald-400 font-semibold">
                    <span>ACH Bank Pay Surcharge:</span>
                    <span className="font-mono">$0.00 (Zero Fee)</span>
                  </div>
                )}
                ...
                {hasMTR && (
                  <p className="text-[10px] text-amber-400/80 pt-1">⚑ MTR / Heat Cert packet required — included in this order</p>
                )}
              </div>

              {/* ── Payment Method Selector ────────────────────── */}
              <div>
                <label className="block text-slate-300 font-semibold text-xs mb-2 uppercase tracking-wider">
              ...
              {/* ── Surcharge / Savings Banner ─────────────────── */}
              {paymentType === 'card' ? (
                <div className="rounded-xl border-2 border-amber-500/60 bg-amber-950/40 p-3 text-xs text-amber-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3 animate-fadeIn">
                  <div className="flex items-start gap-2.5">
                    <AlertTriangle className="h-5 w-5 text-amber-400 shrink-0 mt-0.5" />
                    <div>
                      <strong className="text-amber-300 text-sm block font-black">
                        3.5% Surcharge Applied (+${cardSurchargeAmount.toFixed(2)})
                      </strong>
                      <span className="text-slate-300 text-[11px] leading-relaxed block mt-0.5">
                        Switch to ACH bank pay to eliminate this fee entirely.
                      </span>
                    </div>
                  </div>
                  <button
                  ...
                  >
                    ...
                    <span>Switch &amp; Save ${cardSurchargeAmount.toFixed(2)}</span>
                  </button>
                </div>
              ) : (
                <div className="rounded-xl border border-emerald-500/50 bg-emerald-950/30 p-3 text-xs text-emerald-300 flex items-center justify-between animate-fadeIn">
                  <div className="flex items-center gap-2.5">
                    <ShieldCheck className="h-5 w-5 text-emerald-400 shrink-0" />
                    <div>
                      <strong className="text-emerald-200 font-bold block">
                        ✓ Zero Processing Surcharge — Saved ${cardSurchargeAmount.toFixed(2)}
                      </strong>
                      <span className="text-slate-300 text-[11px]">
                        Direct bank connection via Stripe ACH with Plaid instant verification.
                      </span>
                    </div>
                  </div>
                  <span className="rounded-full bg-emerald-500/20 text-emerald-300 font-mono text-[10px] font-bold px-2.5 py-1 border border-emerald-500/40 shrink-0">
                    ZERO FEE
                  </span>
                </div>
              )}
              ...
              <div>
                <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-3">
                  ...
                  <span className="ml-2 normal-case text-indigo-400 font-normal">— saved to our system for order coordination</span>
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. jmiller@refinery.com"
                      className="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-slate-100 placeholder-slate-500 focus:border-amber-500 focus:outline-none"
                    />
                  </div>
                  <div>
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <input
                      ...
                      placeholder="e.g. Texas City Facility, Gate 4 — ATTN: Maintenance"
                      className="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-slate-100 placeholder-slate-500 focus:border-amber-500 focus:outline-none"
                    />
                  </div>
                </div>
              ...
              <div className="rounded-xl border border-slate-700 bg-slate-800/60 px-4 py-3 flex items-start gap-2.5 text-xs text-slate-400">
                <ExternalLink className="h-4 w-4 text-indigo-400 shrink-0 mt-0.5" />
                <span>
                  After clicking below you'll be taken to{' '}
                  <span className="text-indigo-300 font-semibold">Stripe's secure payment page</span> to enter your{' '}
                  {paymentType === 'card' ? 'card details.' : 'bank account for ACH debit.'}{' '}
                  Your contact info is saved to our system the moment you click — so we can coordinate your order even if you return later.
                </span>
              </div>
                ...
                >
                  {isProcessing ? (
                    <div className="flex items-center gap-2">
                      ...
                      <span>Saving &amp; Connecting to Stripe...</span>
                    </div>
                  ) : (
                    <>
                      <Lock className="h-4 w-4" />
                      <span>
                        Proceed to {paymentType === 'card' ? 'Card' : 'ACH'} Payment — ${finalTotalAmount.toFixed(2)}
                      </span>
