          ...
          <div className="flex bg-slate-900 p-1 rounded-xl border border-slate-800">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-bold transition-all ${
                activeTab === 'kanban' ? 'bg-amber-500 text-slate-950 shadow' : 'text-slate-400 hover:text-white'
              }`}
            >
              Kanban Board
            </button>
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg font-bold transition-all ${
                activeTab === 'clients' ? 'bg-amber-500 text-slate-950 shadow' : 'text-slate-400 hover:text-white'
              }`}
            >
              Clients CRM
            </button>
            ...
            return (
              <div key={stage} className="bg-slate-900/80 border border-slate-800 rounded-2xl p-3.5 space-y-3 flex flex-col">
                <div className="flex items-center justify-between pb-2 border-b border-slate-800">
                  ...
                  <span className="px-2 py-0.5 rounded-full bg-slate-800 text-amber-400 font-black text-[10px] border border-slate-700">
                    {stageOrders.length}
                  </span>
                </div>
                ...
                <div className="space-y-3 flex-1 overflow-y-auto max-h-[600px]">
                  {stageOrders.length === 0 ? (
                    ...
                    stageOrders.map((wo) => (
                      <div
                        key={wo.jobNumber}
                        className="p-3 rounded-xl bg-slate-950 border border-slate-800 hover:border-amber-500/40 transition-all space-y-2 shadow"
                      >
                        <div className="flex justify-between items-start">
                          <span className="font-black text-amber-400 bg-amber-500/10 px-1.5 py-0.5 rounded text-[10px] border border-amber-500/30">
                            {wo.jobNumber}
                          </span>
            ...
            <div className="divide-y divide-slate-800">
              {workOrders.filter(w => w.costing).map((wo) => (
                <div key={wo.jobNumber} className="py-3 flex justify-between items-center text-xs">
                  <div>
                    <span className="font-bold text-amber-400">{wo.jobNumber}</span> &bull; {wo.clientCompanyName}
            ...
            return (
              <div key={cli.companyName} className="p-4 rounded-2xl bg-slate-900 border border-slate-800 space-y-3">
                <div className="flex justify-between items-start">
                  ...
                  <span className="px-2 py-0.5 rounded-full bg-slate-800 text-amber-400 font-bold text-[10px] border border-slate-700">
